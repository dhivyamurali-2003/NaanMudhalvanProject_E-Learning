...
const AddCourse = () => {
   ...
   const handleSubmit = async (e) => {
         ...
         if (key === 'sections') {
            addCourse[key].forEach((section, index) => {
               if (section.S_content instanceof File) {
                  formData.append(`S_content_${index}`, section.S_content);  // Use unique keys for each section file
               }
               formData.append(`S_title_${index}`, section.S_title);
               formData.append(`S_description_${index}`, section.S_description);
            });
         } else {
      ...
      // Debug: log all form data to check what is being appended
